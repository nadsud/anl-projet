...
if(isset($_GET['send']) && $_GET['send'] == 'devis'){
    ...
    $service = '<p>Service sélèctionné : aucun </p>';
    ...
    try {
        ...
        $mail->Username   = $mail_proprietaire;                     // SMTP username
        $mail->Password   = $pass;                                  // SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
    $mail->Port       = $port;                                      // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above
    
        //Recipients
        $mail->setFrom($mail_proprietaire, $titre_message);         
        $mail->addAddress($mail_proprietaire);                      // Mail de destination
